...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Validate parameters
    if (!targetUserId || intensity < 1 || intensity > 100 || duration < 0.1 || duration > 15 || ![0, 1, 2].includes(operation)) {
      return jsonResponse({ 
        success: false, 
    ...
    try {
      ...
      const logEntry: ActivityLogEntry = {
        ...
        executorUsername: user.global_name || user.username,
        executorAvatar: user.avatar ? `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png` : undefined,
        targetUserId,
