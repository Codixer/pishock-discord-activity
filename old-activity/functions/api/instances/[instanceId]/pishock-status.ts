...
async function testPiShockConnection(
  ...
  if (!operateResult.ok) {
    return { ok: false, error: operateResult.error || 'Status test operation failed.' };
  }
  ...
  try {
    ...
    if (encrypted) {
      try {
        ...
        isConnected = connectionResult.ok;
    ...
    return jsonResponse({ 
      ...
      isConnected, 
      lastTested,
      generatedShareCodeHealth: encrypted ? (isConnected ? 'ready' : 'missing_or_invalid') : 'not_configured',
    });
  } catch (error) {
