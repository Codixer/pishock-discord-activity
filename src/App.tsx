...
import { Zap, Shield, Users, Settings, AlertTriangle, Power, FileText, Crown } from 'lucide-react';
...
function MainApp() {
  ...
  const handleMultishock = async (operation: number) => {
    ...
    try {
      ...
      console.log('MULTISHOCK: Starting execution with params:', {
      ...
      if (response.ok) {
        ...
        if (response.status === 403) {
          if (errorData.requiresControllerPlus) {
            ...
            setShowControllerPlusPurchase(true);
            return; // Don't throw error, just show warning and upgrade prompt
          } else {
