...
import { Users, User, Crown, Zap, ZapOff, Smartphone, Lock, ExternalLink } from 'lucide-react';
import { InstanceData } from '../hooks/useInstanceData';
import { useNavigationControl } from './NavigationManager';
...
export function EnhancedUserSelector({ 
  ...
  // Generate shareable link for current user selection
  const generateShareableLink = (user: any) => {
    const shareableUrl = updateUrl({
  ...
  // Handle user selection with URL update
  const handleUserSelect = (user: any) => {
    onUserSelect(user);
    
    // Update URL with selected user for deep linking
    updateUrl({
      selectedUserId: user.id,
      compactMode: isCompactMode
    });
                  ...
                  <div className={`${isCompactMode ? 'p-2' : 'p-2 sm:p-3'} rounded-lg border bg-blue-900/20 border-blue-500/30`}>
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {!isCompactMode && userPiShockStatus[currentUser.id] && (
                    ...
                    {otherParticipants.map((member) => {
                      ...
                      return (
                        <div key={member.id} className="relative group">
                          ...
                          {/* Share button for selected user */}
                          {selectedUser?.id === member.id && !isDisabled && !isCompactMode && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                generateShareableLink(member);
                              }}
                              className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 bg-purple-600 hover:bg-purple-700 rounded text-white"
