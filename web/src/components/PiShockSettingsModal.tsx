...
import { Settings, X, Save, Loader, ExternalLink, Wifi, RefreshCw, LogIn } from 'lucide-react';
import { DiscordSDK } from '@discord/embedded-app-sdk';
import { usePiShockLogin } from '../hooks/usePiShockLogin';
...
export function PiShockSettingsModal({ 
  ...
  const handleLoginSuccess = useCallback(async (result: { userId: number; token: string; username?: string }) => {
    setApiKey(result.token);
    if (result.username) setUsername(result.username);
    setFormError(null);
    setConnectionStatus({ connected: false, message: 'Login received — select a shocker and save', color: 'yellow' });
    try {
      const refreshRes = await authFetch(`${getApiBaseUrl()}/users/${currentUser.id}/pishock-settings`, {
        method: 'PUT',
        headers: {
      ...
      if (refreshRes.ok && Array.isArray(refreshData.availableShockers)) {
        setAvailableShockers(refreshData.availableShockers);
        if (refreshData.availableShockers.length === 1) {
          setSelectedShockerId(String(refreshData.availableShockers[0].id));
        }
      }
    } catch (error) {
      console.error('Failed to refresh shockers after login:', error);
    }
  }, [auth, authFetch, currentUser, username]);

  const { startLogin, isLoggingIn } = usePiShockLogin({
    isEmbedded,
    openExternalLink: (url) => discordSdk.commands.openExternalLink({ url }),
      ...
      <div className="bg-gray-900 rounded-2xl border border-white/20 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-cyan-950/30 border border-cyan-500/40 rounded-lg">
              <p className="text-sm text-cyan-100 mb-3">
                Connect with your PiShock account using the official login flow (Discord OAuth via login.pishock.com).
              </p>
              <button
                type="button"
                onClick={() => void startLogin()}
                disabled={loadingData || isLoggingIn}
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#79c6db] hover:bg-[#68b5ca] text-gray-900 font-semibold rounded-lg transition-colors disabled:opacity-50"
              >
                {isLoggingIn ? <Loader className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />}
                {isLoggingIn ? 'Waiting for PiShock login…' : 'Login with PiShock'}
              </button>
              <p className="text-xs text-gray-400 mt-2">Opens login.pishock.com — credentials are relayed via WebSocket.</p>
            </div>

            <div className="text-center text-xs text-gray-500 uppercase tracking-widest">or manual entry</div>
